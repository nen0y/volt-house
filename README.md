...
Доступ PostgreSQL також задається тільки в кореневому `.env`: `POSTGRES_DB`, `POSTGRES_USER`, `POSTGRES_PASSWORD` і `DATABASE_URL`. Docker Compose не має резервних логінів або паролів і зупиниться з помилкою, якщо якогось значення немає. Якщо пароль містить спеціальні символи, у `DATABASE_URL` він має бути URL-кодований.
