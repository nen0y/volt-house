# E-Kit — інтернет-магазин (фронт + бек + база)

Монорепозиторій:

- **`front/`** — Next.js вітрина (магазин)
- **`back/`** — REST API + адмін-панель + CRM на **PostgreSQL**
...
Фронт сам проксіює `/api`, `/uploads` і `/admin` на бекенд — тож назовні дивиться **лише один порт 3000** (бекенд `back` живе всередині docker-мережі, без публічного порту; без CORS). PostgreSQL зберігається у волюмі `pgdata`, а фото та резервна стара SQLite — у `backdata`; усе переживає перезапуски.
...
> Для повного бекапу збережи волюми `pgdata` (PostgreSQL) і `backdata` (фото та резервна стара SQLite). Під час першого запуску наявний контент зі SQLite переноситься в PostgreSQL автоматично й лише один раз.
