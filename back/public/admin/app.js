...
(function () {
  ...
  function renderFinanceLedger() {
    ...
    const receivedIncomes = financeCache.incomes.filter((income) => income.paymentStatus === "received");
    const receivedTotal = receivedIncomes.reduce((sum, income) => sum + Number(income.amount), 0);
    const unassignedTotal = receivedIncomes.filter((income) => !income.heldBy).reduce((sum, income) => sum + Number(income.amount), 0);
    const expenseTotal = financeCache.expenses.reduce((sum, expense) => sum + Number(expense.amount), 0);
    receivedIncomes.forEach((income) => {
      income.shares.forEach((share) => { incomeShares[share.participantId] += Number(income.amount) * Number(share.percent) / 100; });
      if (income.heldBy) actual[income.heldBy] += Number(income.amount);
    });
    financeCache.expenses.forEach((expense) => { paid[expense.participantId] += Number(expense.amount); actual[expense.participantId] -= Number(expense.amount); });
    (financeCache.transfers || []).forEach((transfer) => { actual[transfer.fromParticipantId] -= Number(transfer.amount); actual[transfer.toParticipantId] += Number(transfer.amount); });
    ...
    const statusLabel = { expected: "Очікуємо оплату", post: "На пошті", received: "Отримано" };
    ...
    $("financeTransferBody").innerHTML = (financeCache.transfers || []).length ? `<table><thead><tr><th>Дата</th><th>Хто передав</th><th>Кому</th><th>Сума</th><th>Спосіб</th><th></th></tr></thead><tbody>${financeCache.transfers.map((transfer) => `<tr><td>${esc(uaDate(transfer.date))}</td><td>${esc(name(transfer.fromParticipantId))}</td><td>${esc(name(transfer.toParticipantId))}</td><td><strong>${money(transfer.amount)}</strong></td><td>${esc(methodLabel[transfer.method])}${transfer.notes ? `<div class="muted">${esc(transfer.notes)}</div>` : ""}</td><td><button class="btn-sm btn-danger" data-del-transfer="${esc(transfer.id)}">Видалити</button></td></tr>`).join("")}</tbody></table>` : `<div class="empty">Передач між учасниками ще не було</div>`;
  ...
  function financeIncomeModal(existing) {
    const item = existing || { purpose: "", customer: "", amount: "", date: todayInKyiv(), notes: "", paymentStatus: "expected", paymentMethod: "card", heldBy: null, shares: financeCache.participants.map((person, index) => ({ participantId: person.id, percent: index === 0 ? 33.34 : 33.33 })) };
    ...
    const updateHolder = () => { $("fin_income_holder_wrap").style.display = $("fin_income_status").value === "received" ? "block" : "none"; };
    ...
    $("fin_income_save").addEventListener("click", async () => {
      ...
      const body = { purpose: $("fin_income_purpose").value.trim(), customer: $("fin_income_customer").value.trim(), amount: Number($("fin_income_amount").value), date: $("fin_income_date").value, notes: $("fin_income_notes").value, paymentStatus: $("fin_income_status").value, paymentMethod: $("fin_income_method").value, heldBy: $("fin_income_status").value === "received" ? ($("fin_income_holder").value || null) : null, shares };
      if (!body.purpose || !body.date || $("fin_income_amount").value === "") return $("fin_income_error").textContent = "Заповніть призначення, дату і суму";
      if (body.paymentStatus === "received" && !body.heldBy) return $("fin_income_error").textContent = "Вкажіть, у кого знаходяться отримані кошти";
  ...
  function financeTransferModal() {
    ...
    $("fin_transfer_save").addEventListener("click", async () => {
      const body = { fromParticipantId: $("fin_transfer_from").value, toParticipantId: $("fin_transfer_to").value, amount: Number($("fin_transfer_amount").value), date: $("fin_transfer_date").value, method: $("fin_transfer_method").value, notes: $("fin_transfer_notes").value };
